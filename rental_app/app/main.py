...
from .routes import pages, equipment, auth as auth_routes
...
app = FastAPI(title="RentalSmz")

# ВАЖНО: абсолютный путь, чтобы /static всегда работал в докере и локально
app.mount("/static", StaticFiles(directory=str(STATIC_DIR)), name="static")

# HTML страницы (меню/логин/регистрация/каталог/дашборд)
...
async def remember_login_mw(request: Request, call_next):
    # если уже есть sid — ничего не делаем
    ...
    try:
        if kind == "client":
            obj = db.execute(
                select(models.Client).where(models.Client.client_id == user_id)
            ).scalar_one_or_none()
            ok = bool(obj and obj.is_active)
        else:
            obj = db.execute(
                select(models.Employee).where(models.Employee.employee_id == user_id)
            ).scalar_one_or_none()
            ok = bool(obj and obj.is_active)
    finally:
        db.close()
...
# Нормальная ручка /auth/me (чтобы UI мог узнавать кто вошёл)
